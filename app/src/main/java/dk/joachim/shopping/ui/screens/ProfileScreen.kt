...
fun ProfileScreen(
    ...
    if (showLogoutConfirmation) {
        AlertDialog(
            ...
            title = { Text("Log ud") },
            text = { Text("Alle lokale data slettes. Du kan logge ind igen med din e-mail og aktiveringskode.") },
            confirmButton = {
                Button(
                    onClick = { showLogoutConfirmation = false; viewModel.logout() },
                    colors = ButtonDefaults.buttonColors(containerColor = MaterialTheme.colorScheme.error),
                ) { Text("Log ud") }
            },
            dismissButton = {
                TextButton(onClick = { showLogoutConfirmation = false }) { Text("Annuller") }
            },
        )
    }
